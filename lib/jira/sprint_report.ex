defmodule Jira.SprintReport do
  ...
  def completed_points(%{"contents" => %{"completedIssuesEstimateSum" => %{"value" => value}}})
  ...
  def committed_points(%{
        "contents" => %{
          "issueKeysAddedDuringSprint" => scope_change_list,
          "puntedIssues" => removed_issues_list,
          "issuesNotCompletedInCurrentSprint" => incomplete_issues_list,
          "completedIssues" => completed_issues_list
        }
      }) do
  ...
  def added_scope_ticket_keys(%{
        "contents" => %{"issueKeysAddedDuringSprint" => scope_change_list}
      }) do
    scope_change_list
    |> Map.keys()
  end
  ...
  def estimate_statistic_from_sprint_report_ticket_info(%{
        "estimateStatistic" => %{"statFieldValue" => %{"value" => value}}
      })
      when is_number(value),
      do: value
